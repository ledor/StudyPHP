...
function showtop10rcb()
{
	...
	if($result=mysql_query("SELECT max( rcb ) AS rcb, name,id
		...
		WHERE onhold=0  AND used =1 and B.program_id = A.id and A.hyip_status in (1,2)
		GROUP BY name
...
function showtop10monitored()
{
	if($result=mysql_query('select *, (to_days(now()) - to_days(date_added)) as monitored
			from `hl_listings` where status=1 and hyip_status in (1,2) order by monitored desc limit 0,10'))
	{
		if (mysql_num_rows($result) > 0) 
		{
